...
export default function AccountsPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h2 className="text-2xl font-semibold">Accounts</h2><Button onClick={() => setShowForm(true)}>Add account</Button></div>
    <Card><CardHeader><CardTitle className="text-base">Account list</CardTitle></CardHeader><CardContent className="p-0 overflow-x-auto"><Table><THead><TR><TH>Name</TH><TH>Currency</TH><TH>Type</TH><TH className="text-right">Initial Balance</TH><TH>Actions</TH></TR></THead><TBody>{accounts.map(a => <TR key={a.ID}><TD>{a.Name}</TD><TD>{a.Currency}</TD><TD>{a.Type}</TD><TD className="text-right">{fmt(a.InitialBalanceMinor)}</TD><TD><div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => edit(a)}>Edit</Button><Button size="sm" variant="destructive" onClick={() => remove(a.ID)}>Delete</Button></div></TD></TR>)}</TBody></Table></CardContent></Card>
